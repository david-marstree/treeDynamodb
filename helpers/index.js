...
const getAttributeValue = (value) => {
    let obj = {};
    if (!!dataType(value)) { // Basic data type
        obj[dataType(value)] = (dataType(value) === 'NULL') ? true : value;
        return obj;
    }

    if (_.isArray(value)) {
        if (_.every(value, (v) => dataType(v) === 'B')) { //Binary Set
            obj['BS'] = value;
            return obj;
        } else if (_.every(value, (v) => dataType(v) === 'N')) { //number set
            obj['NS'] = value;
            return obj;
        } else if (_.every(value, (v) => dataType(v) === 'S')) { //string set
            obj['SS'] = value;
            return obj;
        } else { //list
...
const getValue = (attributeValue) => {
    ...
    if (_.isPlainObject(attributeValue)) {
        if (_.keys(attributeValue).length > 1) {
            ...
            return _.reduce(attributeValue, (prev, attrV, attrKey) => {
            ...
            if (_.includes(['B', 'BOOL', 'BS', 'L', 'M', 'N', 'NS', 'NULL', 'S', 'SS'], key)) {
                return getValue(attributeValue[key]);
            } else {
                if (_.isPlainObject(attributeValue[key]) || _.isArray(attributeValue[key])) {
